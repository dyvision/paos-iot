...
function update_sys(){
    var xmlhttp = new XMLHttpRequest();
    if (document.getElementById('update').innerHTML.toLowerCase().indexOf("- Updated. Restarting...") === -1) {
        document.getElementById('update').innerHTML = document.getElementById('update').innerHTML + ' - Updated. Restarting...'
    } else {}
    xmlhttp.open("GET", "api/update.php", true);
    xmlhttp.setRequestHeader("Content-Type", "application/json");
    xmlhttp.send();
}